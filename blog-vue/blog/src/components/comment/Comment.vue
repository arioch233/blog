    ...
    <div class="comment-input-wrapper">
      <div style="display: flex">
        <v-avatar size="40" class="mt-2">
          ...
          />
          <img v-else :src="this.$store.state.avatar" />
        </v-avatar>
      ...
      >
        ...
        <div class="comment-meta">
          ...
          <div class="comment-user">
            ...
            <span class="blogger-tag" v-if="item.email === 1">博主</span>
          </div>
          <!-- 信息 -->
...
export default {
  ...
  data() {
    return {
      ...
      rules_nickname: [
        (value) => !!value || "昵称不能为空",
        (value) => (value && value.length >= 3) || "昵称至少大于3个字符",
      ],
      email: "",
      rules_email: [
        (value) => !!value || "邮箱不能为空",
  ...
  methods: {
    ...
    listComments() {
      // 获取游客信息
      this.request.get("/tourist").then((data) => {
        if (data.data.id !== null) {
          this.$store.state.avatar = data.data.touristAvatar;
          this.nickname = data.data.touristNickname;
          this.email = data.data.touristEmail;
          // 储存昵称和邮箱至 store
    ...
    insertComment() {
      if (this.$store.state.userId == null) {
        let touristInfo = {
          touristNickname: this.nickname,
          touristEmail: this.email,
          touristAvatar: this.$store.state.avatar,
        };
        this.request.post("/tourist", touristInfo);
      }
      //判空
      ...
      this.commentContent = "";
      this.request.post("/comments", comment).then((data) => {
        if (data.code === 200) {
          // 查询最新评论
          this.current = 1;
          this.listComments();
    ...
    like(comment) {
      ...
      // 发送请求
      this.request.post("/comments/" + comment.id + "/like").then((data) => {
        if (data.code === 200) {
          // 判断是否点赞
