        ...
        <div class="blog-wrapper">
          ...
          <v-card class="blog-card animated zoomIn mt-5 big">
            <div class="web-info-title">
              <v-icon size="24">mdi-new-box</v-icon>
              最新文章
            </div>
            <router-link
              style="display: inline-block; font-size: 0.875rem; width: 100%"
              :to="'/articles/' + item.id"
              class="mr-1"
              v-for="item of newArticleTitle"
              :key="item.id"
            >
              <v-list-item style="padding: 0">
                <v-list-item-content style="padding: 0">
                  <v-list-item-title>
                    <v-icon color="#E6A23C" size="10">mdi-gate-buffer</v-icon>
                    {{ item.articleTitle }}
                  </v-list-item-title>
                </v-list-item-content>
              </v-list-item>
...
export default {
  ...
  data() {
    return {
      ...
      articleList: [
        // {
        //   articleCover:
        //     "https://img-blog.csdnimg.cn/20201224201748118.jpg#pic_center",
        //   articleTitle: "测试标题",
        //   categoryId: 1,
        //   isTop: 0,
        //   createTime: "",
  ...
  methods: {
    ...
    listArticlesNew() {
      this.request.get("/article/new").then((data) => {
        this.newArticleTitle = data.data;
      });
    },
    listHomeTalks() {
      this.request.get("/home/talks").then(({ data }) => {
