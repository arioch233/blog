    ...
    <v-row class="article-container">
      <v-col md="9" cols="12">
        <v-card class="article-wrapper">
          ...
          <!-- 评论 -->
          <comment
            v-if="this.$store.state.blogInfo.websiteConfig.isComment === 1"
            :type="commentType"
...
export default {
  ...
  methods: {
    getArticle() {
      ...
      this.request
        ...
        .then((data) => {
          ...
          this.$nextTick(() => {
            ...
            // 判断文章阅读量
            if (this.wordNum >= 10000) {
              this.article.viewsCount = "大";
            } else if (this.wordNum < 10000 && this.wordNum >= 5000) {
              this.article.viewsCount = "中";
            } else {
              this.article.viewsCount = "小";
            }
            // 添加代码复制功能
      ...
      // 查询点赞信息
      this.request.get("/article/like").then((data) => {
        this.$store.commit("saveLikeInfo", data.data);
      });
    },
    like() {
      //发送请求
      this.request.put("/article/like/" + this.article.id).then((data) => {
        if (data.code === 200) {
          //判断是否点赞
          if (
